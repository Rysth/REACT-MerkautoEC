...
function Auto({ register, errors }) {
	...
	function moveActualCircle(event) {
		...
		const originalCircle = document.querySelector(`.status-circle.placed[data-status="${data}"]`);
		if (originalCircle) {
			const auto = document.querySelector('.auto');
			const rect = auto.getBoundingClientRect();
	...
	const drop = (event) => {
		event.preventDefault();
		const data = event.dataTransfer.getData('text');
		const originalCircle = document.querySelector(`.status-circle[data-status="${data}"]`);
		if (originalCircle) {
			const auto = document.querySelector('.auto');
			const rect = auto.getBoundingClientRect();
			const x = event.clientX - rect.left - initialX;
